...
var others = /* Wildcard */Block.variant("Wildcard", 1, [(function (raw) {
...
var partial_arg$1 = Util$AgdaMode.Re_[/* choice */1];
...
function allGoalsWarnings(title, body) {
  var partial_arg = /* array */[/* Regex */Block.variant("Regex", 0, [
        (/((?:\n|.)*\S+)\s*\[ at (.+):(?:(\d+)\,(\d+)\-(\d+)\,(\d+)|(\d+)\,(\d+)\-(\d+)) \]/),
        occurence
      ])];
  var partial_arg$1 = Util$AgdaMode.Re_[/* choice */1];
  var metas = $$Array.map((function (param) {
          return partial_arg$1(partial_arg, param);
