...
function reducer(editor, action, state) {
  if (typeof action === "number") {
    ...
    switch (action.tag | 0) {
      ...
      case 1 : 
          var match$2 = Buffer$AgdaMode.next(state[/* buffer */2], action[1]);
...
function InputMethod(Props) {
  ...
  var send = match[1];
  var state = match[0];
  var setDebug = React.useContext(Type__View$AgdaMode.Debug[/* setDebug */1]);
  React.useEffect((function () {
          if (atom.inDevMode()) {
            Curry._1(setDebug, /* record */Block.record(["inputMethod"], [Block.record([
                        "activated",
                        "markers",
                        "buffer"
                      ], [
