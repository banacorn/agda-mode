...
function before_each(param) {
  console.log("======== before each =============");
  return Curry._1(openAsset, "Temp.agda").then((function (editor) {
                console.log("[ before each ] is empty:", Rebase.$$String[/* isEmpty */5](editor.getText()));
                return Promise.resolve(/* () */0);
              }));
}
...
function after_each(param) {
  ...
  var clearAllFiles = function (param) {
    return Curry._1(openAsset, "Temp.agda").then((function (editor) {
                  ...
                  while(true) {
                    editor.setText("");
                    if (Rebase.$$String[/* isEmpty */5](editor.getText())) {
                      return editor.save();
                    } else {
                      console.log("[ after each ] cleanup failed");
                      _param = /* () */0;
                      continue ;
                    }
                  };
  ...
  return clearAllFiles(/* () */0).then(destroyAllTextEditors);
}
...
function insertUntilSuccess(text, instance) {
  ...
  return editor.save().then((function (param) {
                ...
                if (before !== after) {
                  console.log("[ inserted ]", text, before, after);
                  return Promise.resolve(instance);
                } else {
                  console.log("[ insertion failed ]", after);
...
function backspace(instance) {
  var backspaceUntilSuccess = function (param) {
    ...
    return instance[/* editors */1][/* source */1].save().then((function (param) {
                  ...
                  if (before !== after) {
                    console.log("[ backspaced ]", before, after);
                    return Promise.resolve(/* () */0);
                  } else {
                    console.log("[ backspace failed]", after);
