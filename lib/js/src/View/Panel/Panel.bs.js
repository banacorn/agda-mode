// Generated by BUCKLESCRIPT VERSION 5.0.3, PLEASE EDIT WITH CARE
'use strict';

var Block = require("bs-platform/lib/js/block.js");
var Curry = require("bs-platform/lib/js/curry.js");
var React = require("react");
var ReactDom = require("react-dom");
var Caml_option = require("bs-platform/lib/js/caml_option.js");
var ReasonReact = require("reason-react/lib/js/src/ReasonReact.js");
var Body$AgdaMode = require("./Body.bs.js");
var Util$AgdaMode = require("../../Util.bs.js");
var Event$AgdaMode = require("../../Util/Event.bs.js");
var ReasonReactCompat = require("reason-react/lib/js/src/ReasonReactCompat.js");
var Dashboard$AgdaMode = require("./Dashboard.bs.js");
var MiniEditor$AgdaMode = require("../MiniEditor.bs.js");
var InputMethod$AgdaMode = require("./InputMethod.bs.js");
var SizingHandle$AgdaMode = require("./SizingHandle.bs.js");

function Panel(Props) {
  var editors = Props.editors;
  var element = Props.element;
  var onMountAtChange = Props.onMountAtChange;
  var body = Props.body;
  var header = Props.header;
  var mountAt = Props.mountAt;
  var mode = Props.mode;
  var hidden = Props.hidden;
  var settingsView = Props.settingsView;
  var isPending = Props.isPending;
  var isActive = Props.isActive;
  var onInquireQuery = Props.onInquireQuery;
  var onEditorRef = Props.onEditorRef;
  var editorPlaceholder = Props.editorPlaceholder;
  var editorValue = Props.editorValue;
  var interceptAndInsertKey = Props.interceptAndInsertKey;
  var activateInputMethod = Props.activateInputMethod;
  var onSettingsViewToggle = Props.onSettingsViewToggle;
  var match = React.useState((function () {
          return 170;
        }));
  var setMaxHeight = match[1];
  var match$1 = React.useState((function () {
          return false;
        }));
  var setInputMethodActivation = match$1[1];
  var mountAtBottom;
  mountAtBottom = mountAt.tag ? false : true;
  var style = mountAtBottom ? ({
        maxHeight: String(match[0]) + "px"
      }) : undefined;
  var className = Curry._1(Util$AgdaMode.ClassName[/* serialize */2], Util$AgdaMode.ClassName[/* addWhen */1]("hidden", hidden, /* [] */0));
  var tmp = {
    className: "agda-body-container"
  };
  if (style !== undefined) {
    tmp.style = Caml_option.valFromOption(style);
  }
  return ReactDom.createPortal(React.createElement("section", {
                  className: className
                }, React.createElement("section", {
                      className: "panel-heading agda-header-container"
                    }, React.createElement(SizingHandle$AgdaMode.make, {
                          onResizeStart: (function (height) {
                              return Curry._1(setMaxHeight, (function (param) {
                                            return height;
                                          }));
                            }),
                          onResizeEnd: (function (height) {
                              setTimeout((function (param) {
                                      Curry._1(setMaxHeight, (function (param) {
                                              return height;
                                            }));
                                      atom.config.set("agda-mode.maxBodyHeight", String(height));
                                      return /* () */0;
                                    }), 0);
                              return /* () */0;
                            }),
                          mountAtBottom: mountAtBottom
                        }), React.createElement(InputMethod$AgdaMode.make, {
                          editors: editors,
                          interceptAndInsertKey: interceptAndInsertKey,
                          activateInputMethod: activateInputMethod,
                          onActivationChange: (function (activated) {
                              return Curry._1(setInputMethodActivation, (function (param) {
                                            return activated;
                                          }));
                            }),
                          isActive: isActive
                        }), React.createElement(Dashboard$AgdaMode.make, {
                          header: header,
                          hidden: match$1[0],
                          isPending: isPending,
                          mountAt: mountAt,
                          settingsView: settingsView,
                          onMountAtChange: onMountAtChange,
                          onSettingsViewToggle: onSettingsViewToggle
                        })), React.createElement("section", tmp, React.createElement(Body$AgdaMode.make, {
                          body: body,
                          hidden: mode !== /* Display */0
                        }), React.createElement(MiniEditor$AgdaMode.make, {
                          value: editorValue,
                          placeholder: editorPlaceholder,
                          hidden: mode !== /* Inquire */1,
                          grammar: "agda",
                          onConfirm: (function (result) {
                              return Event$AgdaMode.emitOk(result, onInquireQuery);
                            }),
                          onCancel: (function () {
                              Event$AgdaMode.emitError(/* Cancelled */0, onInquireQuery);
                              return /* () */0;
                            }),
                          onEditorRef: onEditorRef
                        }))), element);
}

var component = ReasonReact.statelessComponent("Panel");

function make(editors, element, onMountAtChange, body, header, mountAt, mode, hidden, settingsView, isPending, isActive, onInquireQuery, onEditorRef, editorPlaceholder, editorValue, interceptAndInsertKey, activateInputMethod, onSettingsViewToggle, children) {
  return ReasonReactCompat.wrapReactForReasonReact(Panel, {
              editors: editors,
              element: element,
              onMountAtChange: onMountAtChange,
              body: body,
              header: header,
              mountAt: mountAt,
              mode: mode,
              hidden: hidden,
              settingsView: settingsView,
              isPending: isPending,
              isActive: isActive,
              onInquireQuery: onInquireQuery,
              onEditorRef: onEditorRef,
              editorPlaceholder: editorPlaceholder,
              editorValue: editorValue,
              interceptAndInsertKey: interceptAndInsertKey,
              activateInputMethod: activateInputMethod,
              onSettingsViewToggle: onSettingsViewToggle
            }, children);
}

var Jsx2 = /* module */Block.localModule([
    "component",
    "make"
  ], [
    component,
    make
  ]);

var make$1 = Panel;

exports.make = make$1;
exports.Jsx2 = Jsx2;
/* component Not a pure module */
