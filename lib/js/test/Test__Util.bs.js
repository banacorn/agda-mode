...
function getPanelAtBottom(instance) {
  var panels = Rebase.$$Array[/* filter */10]((function (elem) {
          return elem.className === "agda-mode";
  ...
  var agdaPanel = Rebase.$$Array[/* filter */10]((function (elem) {
  ...
  var match = Rebase.$$Array[/* get */17](agdaPanel, 0);
  if (match !== undefined) {
...
function deactivate(param) {
  ...
  AgdaMode$AgdaMode.deactivate(/* () */0);
...
function dispatch($$event, editor) {
  return getInstance(editor).then((function (instance) {
                var onDispatch = Event$AgdaMode.once(instance[/* onDispatch */10]);
                var match = atom.commands.dispatch(atom.views.getView(editor), $$event);
                if (match !== undefined) {
...
function press$prime(element, key) {
