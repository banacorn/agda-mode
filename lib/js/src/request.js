...
exports.whyInScope = (expr, hole) => buildRequest('NonInteractive', `Cmd_why_in_scope ${GoalRE.index(hole)} noRange \"${util_1.parseInputContent(expr)}\"`);
exports.whyInScopeGlobal = (expr) => buildRequest('None', `Cmd_why_in_scope_toplevel \"${util_1.parseInputContent(expr)}\"`);
exports.inferType = (normalization, expr, hole) => buildRequest('NonInteractive', `Cmd_infer ${normalization} ${GoalRE.index(hole)} noRange \"${expr}\"`);
exports.inferTypeGlobal = (normalization, expr) => buildRequest('None', `Cmd_infer_toplevel ${normalization} \"${expr}\"`);
exports.moduleContents = (normalization, expr, hole) => buildRequest('NonInteractive', `Cmd_show_module_contents ${normalization} ${GoalRE.index(hole)} noRange \"${expr}\"`);
...
exports.refine = (hole) => buildRequest('NonInteractive', conn => (`Cmd_refine_or_intro False ${GoalRE.index(hole)} ${buildRange(conn, hole)} \"${GoalRE.getContent(hole)}\"`));
exports.auto = (hole) => buildRequest('NonInteractive', conn => (`Cmd_auto ${GoalRE.index(hole)} ${buildRange(conn, hole)} \"${GoalRE.getContent(hole)}\"`));
exports.makeCase = (hole) => buildRequest('NonInteractive', conn => (`Cmd_make_case ${GoalRE.index(hole)} ${buildRange(conn, hole)} \"${GoalRE.getContent(hole)}\"`));
exports.goalType = (normalization, hole) => buildRequest('NonInteractive', () => (`Cmd_goal_type ${normalization} ${GoalRE.index(hole)} noRange \"\"`));
exports.context = (normalization, hole) => buildRequest('NonInteractive', () => (`Cmd_context ${normalization} ${GoalRE.index(hole)} noRange \"\"`));
exports.goalTypeAndContext = (normalization, hole) => buildRequest('NonInteractive', () => (`Cmd_goal_type_context ${normalization} ${GoalRE.index(hole)} noRange \"\"`));
exports.goalTypeAndInferredType = (normalization, hole) => buildRequest('NonInteractive', () => (`Cmd_goal_type_context_infer ${normalization} ${GoalRE.index(hole)} noRange \"${GoalRE.getContent(hole)}\"`));
exports.gotoDefinition = (expr, hole) => buildRequest('NonInteractive', `Cmd_why_in_scope ${GoalRE.index(hole)} noRange \"${expr}\"`);
