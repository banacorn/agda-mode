...
function Root(Props) {
  ...
  Hook$AgdaMode.useDidUpdateEffect2((function (param) {
          var match = state[/* mountAt */0];
          var match$1 = state[/* isActive */1];
          if (match.tag) {
            if (match$1) {
              Event$AgdaMode.emitOk(Caml_option.some(match[0][/* element */0]), handles[/* onPanelActivationChange */4]);
            } else {
              Event$AgdaMode.emitOk(undefined, handles[/* onPanelActivationChange */4]);
            }
          } else if (match$1) {
            Event$AgdaMode.emitOk(Caml_option.some(match[0]), handles[/* onPanelActivationChange */4]);
          } else {
            Event$AgdaMode.emitOk(undefined, handles[/* onPanelActivationChange */4]);
          }
          return undefined;
