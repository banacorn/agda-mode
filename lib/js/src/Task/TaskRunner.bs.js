...
function run(instance, errorHandler, tasks) {
  ...
  var runTask = function (task) {
    ...
    if (typeof task === "number") {
      ...
      switch (task.tag | 0) {
        ...
        case /* SendRequest */6 :
            return runTasks(sendRequest(instance, task[0]));
  ...
  var postponeDispatchCommand = function (tasks) {
    var isDispatchCommand = function (param) {
      if (typeof param === "number" || param.tag !== /* DispatchCommand */5) {
        return false;
      } else {
        return true;
      }
    };
    var dispatches = Rebase.List.filter(isDispatchCommand, tasks);
    var otherTasks = Rebase.List.filter(Curry._2(Rebase.Fn.$great$great, isDispatchCommand, (function (prim) {
                return !prim;
              })), tasks);
    if (!Rebase.List.isEmpty(dispatches)) {
      console.log(Rebase.$$Array.fromList(tasks), Rebase.$$Array.fromList(Rebase.List.concat(otherTasks, dispatches)));
    }
    return Rebase.List.concat(otherTasks, dispatches);
  };
  return runEach(postponeDispatchCommand(tasks));
}
            ...
            if (param.tag) {
              ...
              if (match) {
                ...
                if (match$1.tag) {
                  ...
                  var response = match$1[0];
                  console.log(" >>> ", Response$AgdaMode.toString(response));
                  var result = Instance__TextEditors$AgdaMode.restoreCursorPosition((function (param) {
                          var tasks = Task__Response$AgdaMode.handle(response);
                          return run(instance, errorHandler, tasks);
                        }), instance);
