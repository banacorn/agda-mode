...
var caseSingleHole = parsimmon_1.seq(location, token("Right hand side must be a single hole when making a case").then(token("distinction")), token("when checking that the expression"), trimBeforeAndSkip("has type"), parsimmon_1.all).map(function (result) {
    return {
        type: 13 /* CaseSingleHole */,
        location: result[0],
        expr: result[3],
        exprType: result[4]
    };
});
var patternMatchOnNonDatatype = parsimmon_1.seq(location, token("Cannot pattern match on non-datatype").then(trimBeforeAndSkip("when checking that the expression")), trimBeforeAndSkip("has type"), parsimmon_1.all).map(function (result) {
    return {
        type: 14 /* PatternMatchOnNonDatatype */,
        location: result[0],
