...
exports.searchAbout = (normalization, expr, goal) => buildRequest('NonInteractive', `Cmd_search_about_toplevel ${normalization} ${goal.index} noRange \"${expr}\"`);
exports.searchAboutGlobal = (normalization, expr) => buildRequest('None', `Cmd_search_about_toplevel ${normalization} \"${expr}\"`);
exports.whyInScope = (expr, goal) => buildRequest('NonInteractive', `Cmd_why_in_scope ${goal.index} noRange \"${util_1.parseInputContent(expr)}\"`);
