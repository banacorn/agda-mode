...
class InputMethod {
    ...
    activate() {
        if (!this.activated) {
            ...
            this.core.view.editors.getFocusedEditor().then(editor => {
                ...
                this.textEditorMarkers = ranges.map(range => this.editor.markBufferRange(new atom_1.Range(range.start, range.end), {}));
                // monitors only the first marker
                this.textEditorMarkers[0].onDidChange(this.dispatchEvent);
                // decorations
                this.decorations = this.textEditorMarkers.map((textEditorMarker) => this.editor.decorateMarker(textEditorMarker, {
                    type: 'highlight',
                ...
                this.core.view.store.dispatch(actions_1.INPUT_METHOD.activate());
            });
        }
        else {
    ...
    deactivate() {
        if (this.activated) {
            ...
            this.core.view.store.dispatch(actions_1.INPUT_METHOD.deactivate());
