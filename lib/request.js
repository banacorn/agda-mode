...
const makeRequest = (highlightingLevel, interaction) => (conn) => {
    const highlightingMethod = atom.config.get('agda-mode.highlightingMethod');
    let request;
    if (typeof interaction === 'string') {
        request = `IOTCM \"${conn.filepath}\" ${highlightingLevel} ${highlightingMethod} ( ${interaction} )\n`;
...
exports.load = makeRequest('NonInteractive', (conn) => {
...
exports.abort = makeRequest('NonInteractive', 'Cmd_abort');
exports.compile = makeRequest('NonInteractive', (conn) => {
...
exports.toggleDisplayOfImplicitArguments = makeRequest('NonInteractive', 'ToggleImplicitArgs');
exports.solveConstraints = (normalization) => makeRequest('NonInteractive', `Cmd_solveAll ${normalization}`);
exports.showConstraints = makeRequest('NonInteractive', 'Cmd_constraints');
exports.showGoals = makeRequest('NonInteractive', 'Cmd_metas');
exports.whyInScope = (expr, goal) => makeRequest('NonInteractive', `Cmd_why_in_scope ${goal.index} noRange \"${expr}\"`);
exports.whyInScopeGlobal = (expr) => makeRequest('None', `Cmd_why_in_scope_toplevel \"${expr}\"`);
exports.inferType = (normalization, expr, goal) => makeRequest('NonInteractive', `Cmd_infer ${normalization} ${goal.index} noRange \"${expr}\"`);
exports.inferTypeGlobal = (normalization, expr) => makeRequest('None', `Cmd_infer_toplevel ${normalization} \"${expr}\"`);
exports.moduleContents = (normalization, expr, goal) => makeRequest('NonInteractive', `Cmd_show_module_contents ${normalization} ${goal.index} noRange \"${expr}\"`);
exports.moduleContentsGlobal = (normalization, expr) => makeRequest('None', `Cmd_show_module_contents_toplevel ${normalization} \"${expr}\"`);
exports.computeNormalForm = (computeMode, expr, goal) => makeRequest('NonInteractive', conn => {
...
exports.computeNormalFormGlobal = (computeMode, expr) => makeRequest('None', conn => {
    if (semver.gte(conn.version.sem, '2.5.2')) {
...
exports.give = (goal) => makeRequest('NonInteractive', conn => {
...
exports.refine = (goal) => makeRequest('NonInteractive', conn => (`Cmd_refine_or_intro False ${goal.index} ${buildRange(conn, goal)} \"${goal.getContent()}\"`));
exports.auto = (goal) => makeRequest('NonInteractive', conn => (`Cmd_auto ${goal.index} ${buildRange(conn, goal)} \"${goal.getContent()}\"`));
exports.makeCase = (goal) => makeRequest('NonInteractive', conn => (`Cmd_make_case ${goal.index} ${buildRange(conn, goal)} \"${goal.getContent()}\"`));
exports.goalType = (normalization, goal) => makeRequest('NonInteractive', conn => (`Cmd_goal_type ${normalization} ${goal.index} noRange \"\"`));
exports.context = (normalization, goal) => makeRequest('NonInteractive', conn => (`Cmd_context ${normalization} ${goal.index} noRange \"\"`));
exports.goalTypeAndContext = (normalization, goal) => makeRequest('NonInteractive', conn => (`Cmd_goal_type_context ${normalization} ${goal.index} noRange \"\"`));
exports.goalTypeAndInferredType = (normalization, goal) => makeRequest('NonInteractive', conn => (`Cmd_goal_type_context_infer ${normalization} ${goal.index} noRange \"${goal.getContent()}\"`));
