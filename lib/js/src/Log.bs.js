...
function createEntry(cmd, log) {
  ...
  log[/* entries */1].push(entry);
  return /* () */0;
}

function updateLatestEntry(f, log) {
  var n = Rebase.$$Array[/* length */16](log[/* entries */1]);
  return Rebase.$$Option[/* forEach */8](f, Rebase.$$Array[/* get */17](log[/* entries */1], n - 1 | 0));
}
...
function serialize$1(log) {
  var entries = Rebase.$$String[/* joinWith */11]("\n", Rebase.List[/* fromArray */12](Rebase.$$Array[/* mapi */27](serialize, log[/* entries */1])));
  return "## Parse Log\n" + (String(entries) + "\n");
}

function dump(log) {
  var text = serialize$1(log);
  var itemOptions = {
    initialLine: 0,
    initialColumn: 0,
    split: "left",
