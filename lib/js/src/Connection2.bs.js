...
function run$1(pathAndParams, validator) {
  ...
  var match = $$Promise.pending(/* () */0);
  var resolve = match[1];
  var match$1 = Parser$AgdaMode.commandLine(pathAndParams);
  if (Rebase.$$String.isEmpty(match$1[0])) {
  ...
  Child_process.exec(pathAndParams, (function (error, stdout, stderr) {
          clearTimeout(hangTimeout);
          ...
          var match$1 = Curry._1(validator, stdout.toString());
          if (match$1.tag) {
...
function make(path, args) {
  ...
  var $$process$1 = /* record */Caml_chrome_debugger.record(["contents"], [Caml_chrome_debugger.variant("Connected", 0, [$$process])]);
  var send = function (request) {
    var match = $$process$1[0];
    if (typeof match === "number" || match.tag) {
      return /* Error */Caml_chrome_debugger.variant("Error", 1, [/* NotEstablishedYet */1]);
    } else {
      var payload = Buffer.from(request + "\n");
      match[0].stdin.write(payload);
  ...
  var disconnect = function (param) {
    ...
    if (typeof match === "number") {
      ...
      return Curry._1(match[0][/* acquire */0], /* () */0);
    } else {
      var pending = Resource$AgdaMode.make(/* () */0);
      $$process$1[0] = /* Disconnecting */Caml_chrome_debugger.variant("Disconnecting", 1, [pending]);
      Curry._1(emitter[/* on */3], (function (param) {
              if (param.tag) {
                var tmp = param[0];
                if (typeof tmp === "number" || tmp.tag !== /* ExitedByProcess */2) {
                  return /* () */0;
                } else {
                  Curry._1(emitter[/* destroy */4], /* () */0);
                  $$process$1[0] = /* Disconnected */0;
                  return Curry._1(pending[/* supply */1], /* () */0);
                }
              } else {
                return /* () */0;
              }
            }));
      Curry._3(Rebase.Fn.$great$great, (function (param) {
              return param.kill("SIGTERM");
            }), (function (prim) {
              return /* () */0;
            }), match[0]);
      return Curry._1(pending[/* acquire */0], /* () */0);
    }
  };
