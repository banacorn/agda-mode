...
function unindent(lines) {
  var newLineIndices = $$Array.map((function (param) {
          return param[2];
              ...
              if (sort.test(line) || delimeter.test(line) || reallyLongTermIdentifier.test(line) && Js_option.isSomeValue((function (_, line) {
...
function allGoalsWarnings(title, body) {
      ...
      if (hasWarnings) {
        if (hasErrors) {
          return /* record */Block.record([
                    ...
                    unindent(shitpile.slice(0, indexOfWarnings)),
        ...
        return /* record */Block.record([
                  ...
                  unindent(shitpile.slice(0, indexOfErrors)),
                  [],
                  shitpile.slice(indexOfErrors + 1 | 0)
                ]);
      } else {
  ...
  var x = metas.findIndex((function (s) {
          return Js_option.isSome(Util$AgdaMode.Parser[/* parse */1](outputWithRange, s));
        }));
  var indexOfHiddenMetas = x !== -1 ? x : undefined;
  var interactionMetas = indexOfHiddenMetas !== undefined ? Util$AgdaMode.Parser[/* parseArray */2](outputWithoutRange, metas.slice(0, indexOfHiddenMetas)) : Util$AgdaMode.Parser[/* parseArray */2](outputWithoutRange, metas);
  var hiddenMetas = indexOfHiddenMetas !== undefined ? Util$AgdaMode.Parser[/* parseArray */2](outputWithRange, metas.slice(indexOfHiddenMetas)) : /* array */[];
  return /* record */Block.record([
            "interactionMetas",
            "hiddenMetas",
            "warnings",
...
function goalTypeContext(raw) {
  var partiteGoalTypeContext = function (raw) {
    var lines = raw.split("\n");
    lines.findIndex((function (s) {
            return Js_option.isSome(Js_primitive.null_to_opt(s.match((/^Goal:/))));
          }));
    var indexOfHave = lines.findIndex((function (s) {
    ...
    if (indexOfHave === -1) {
      return /* record */Block.record([
                ...
                $$String.concat("", $$Array.to_list(lines.slice(0, indexOfDelimeter))),
                undefined,
                lines.slice(indexOfDelimeter + 1 | 0)
              ]);
    } else {
      return /* record */Block.record([
                "goal",
                ...
                $$String.concat("", $$Array.to_list(lines.slice(0, indexOfHave))),
                Js_option.some($$String.concat("", $$Array.to_list(lines.slice(indexOfHave, indexOfDelimeter)))),
                lines.slice(indexOfDelimeter + 1 | 0)
              ]);
    }
  };
