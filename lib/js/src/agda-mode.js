...
const commands = [
    ...
    'agda-mode:search-about[Simplified]',
    'agda-mode:search-about[Instantiated]',
    'agda-mode:search-about[Normalised]',
        ...
        subscriptions.add(atom.commands.add('atom-text-editor', command, event => {
            ...
            if (isAgdaEditor(textEditor)) {
                ...
                if (command === 'core:undo') {
                    ...
                    core.commander.dispatch(emacs_1.parseCommand(command));
                }
            }
        }));
    });
...
const config = {
    ...
    inputTriePath: {
        title: 'Custom Unicode Keymap Trie and Lookup',
        description: 'Path to the folder containing your custom unicode bindings and character lookup table, generated using [this keymap tool](https://github.com/banacorn/keymap). This is a temp solution to an uncommon problem (see #79), you likely don\'t have to mess with this config. When you update this option or the files, remember to reload atom',
        type: 'string',
        default: '',
        order: 60
    },
