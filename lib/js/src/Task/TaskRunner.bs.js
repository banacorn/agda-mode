...
function dispatchCommand(command, instance) {
  ...
  return $$Promise.flatMap($$Promise.map(execute(Rebase.$$Array.fromList(Task__Command$AgdaMode.handle(command)), instance), Rebase.List.fromArray), dispatchCommands);
}
