...
var trimBefore = function (s) { return beforePrim(function (x) { return x.trim(); }, s); };
var trimBeforeAndSkip = function (s) { return trimBefore(s).skip(parsimmon_1.string(s)); };
...
var notInScope = parsimmon_1.seq(location.skip(spaces), parsimmon_1.string("Not in scope:").then(spaces).then(trimBeforeAndSkip("at")).skip(parsimmon_1.all)).map(function (result) {
    return {
        type: 0 /* NotInScope */,
        expr: result[1],
        location: result[0]
...
var typeMismatch = parsimmon_1.seq(location.skip(spaces), parsimmon_1.alt(trimBeforeAndSkip("!=<"), trimBeforeAndSkip("=<")), trimBeforeAndSkip("of type"), trimBeforeAndSkip("when checking that the expression"), trimBeforeAndSkip("has type"), parsimmon_1.all).map(function (result) {
    console.log(result);
    return {
        type: 1 /* TypeMismatch */,
        actual: result[1],
...
var errorParser = parsimmon_1.alt(notInScope, typeMismatch);
