...
function getLibraryPath(param) {
  var raw = atom.config.get("agda-mode.libraryPath");
  return Belt_Array.map(Belt_Array.keep(raw, (function (x) {
                    return x !== "";
...
function highlightBackground(editor, kind, range) {
  ...
  return [createMarker("goal-background", range)];
}
...
function overlayText(editor, kind, text, range) {
  ...
  return [createOverlay(text, "goal-index", false, /* tuple */[
                0,
                -1
              ], range)];
}
