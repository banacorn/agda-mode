...
class Editor {
    ...
    protectCursor(callback) {
        ...
        if (goal) {
            ...
            if (isFreshHole) {
                const newPosition = this.translate(atom_1.Point.fromObject(HoleRE.range(goal).start), 3);
                setTimeout(() => {
    ...
    jumpToNextGoal() {
        ...
        const positions = this.goals.map((goal) => {
            return this.translate(atom_1.Point.fromObject(HoleRE.range(goal).start), 3);
        });
    ...
    jumpToGoal(index) {
        ...
        if (goal) {
            const position = this.translate(atom_1.Point.fromObject(HoleRE.range(goal).start), 3);
            this.textEditor.setCursorBufferPosition(position);
            this.focus();
        }
    }
