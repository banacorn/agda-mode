...
function removeBoundary(self) {
  var range = self[/* range */2].translate(new Point(0, 2), new Point(0, -2));
...
function writeLines(self, contents) {
  ...
  if (match !== undefined) {
    ...
    var match$1 = firstRow.match((/^(\s)*/));
    var indentSpaces;
    if (match$1 !== null) {
      var match$2 = Rebase.$$Array[/* get */17](match$1, 0);
      indentSpaces = match$2 !== undefined ? match$2 : "";
    } else {
      indentSpaces = "";
    }
    var indentedContents = Rebase.$$String[/* joinWith */11]("\n", Rebase.List[/* map */0]((function (line) {
                return indentSpaces + line;
              }), contents));
    var match$3 = Rebase.$$Array[/* get */17](rowNumbers, Rebase.$$Array[/* length */16](rowNumbers) - 1 | 0);
    if (match$3 !== undefined) {
      textBuffer.deleteRows(firstRowNumber, match$3);
    }
...
function writeLambda(self, contents) {
  var textBuffer = self[/* textEditor */0].getBuffer();
  var beforeRange = new Atom.Range(textBuffer.getFirstPosition(), self[/* range */2].start);
  var afterRange = new Atom.Range(self[/* range */2].end, textBuffer.getEndPosition());
  self[/* textEditor */0].backwardsScanInBufferRange((/\;\s*|\{\s*/), beforeRange, (function (result) {
          var rewriteRangeStart = result.range.end;
          result.stop();
          self[/* textEditor */0].scanInBufferRange((/\s*\;|\s*\}/), afterRange, (function (result) {
                  var rewriteRangeEnd = result.range.start;
                  result.stop();
                  var rewriteRange = new Atom.Range(rewriteRangeStart, rewriteRangeEnd);
                  self[/* textEditor */0].setTextInBufferRange(rewriteRange, Rebase.$$String[/* joinWith */11](" ; ", contents));
                  return /* () */0;
                }));
          return /* () */0;
        }));
...
function make(textEditor, index, start, end_) {
  var textBuffer = textEditor.getBuffer();
  var startPoint = textBuffer.positionForCharacterIndex(start);
  var endPoint = textBuffer.positionForCharacterIndex(end_);
  var range = new Atom.Range(startPoint, endPoint);
  var content = textBuffer.getTextInRange(range);
  ...
  element.setAttribute("style", "left: " + (String((-match[0] | 0) - 2 | 0) + "ex"));
  element.setAttribute("style", "top: -1.5em");
              ...
              if (deltaLeft !== undefined) {
                ...
                if (exit === 1) {
                  if (deltaRight !== undefined) {
                    ...
                    t[/* range */2] = newRange.translate(new Point(0, left), new Point(0, right$prime));
...
function selectContent(self) {
  ...
  var range = self[/* range */2].translate(new Point(0, 3), new Point(0, -(3 + indexWidth | 0) | 0));
