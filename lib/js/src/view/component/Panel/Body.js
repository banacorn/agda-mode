    ...
    render() {
        ...
        return (React.createElement("section", { className: classes, tabIndex: -1, style: style },
            ...
            emacsAllGoalsWarnings &&
                React.createElement(view_1.default.EventContext.Consumer, null, emitter => (React.createElement(EmacsAllGoalsWarnings, { emacsAllGoalsWarnings: emacsAllGoalsWarnings, emit: (ev, range) => {
                        switch (ev) {
                            case actions_1.EVENT.JUMP_TO_RANGE:
                                emitter.emit(actions_1.EVENT.JUMP_TO_RANGE, toAtomRange(range), toAtomFilepath(range));
                                break;
