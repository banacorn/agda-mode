...
function Settings__Log$Entry(Props) {
  ...
  var rawTexts = Util$AgdaMode.React.manyIn(Rebase.$$Array.map((function (text) {
              return React.createElement("li", undefined, text);
            }), entry.response.rawText), "ol");
  var sexpressions = Util$AgdaMode.React.manyIn(Rebase.$$Array.map((function (text) {
              return React.createElement("li", undefined, Curry._1(Parser$AgdaMode.SExpression.toString, text));
            }), entry.response.sexpression), "ol");
  var responses = Util$AgdaMode.React.manyIn(Rebase.$$Array.map((function (text) {
              return React.createElement("li", undefined, Response$AgdaMode.toString(text));
            }), entry.response.response), "ol");
  var hasError = Rebase.$$Array.length(entry.response.error) > 0;
  var errors = Util$AgdaMode.React.manyIn(Rebase.$$Array.map((function (text) {
              return React.createElement("li", undefined, Parser$AgdaMode.$$Error.toString(text));
            }), entry.response.error), "ol");
...
function Settings__Log(Props) {
  ...
  var entries = Util$AgdaMode.React.manyIn(Rebase.$$Array.map((function (entry) {
