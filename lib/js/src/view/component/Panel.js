...
function mapStateToProps(state) {
    return Object.assign({ mountAt: state.view.mountAt, settingsView: state.view.settingsView, pending: state.protocol.pending, inputMethod: state.inputMethod }, state);
}
        ...
        return (React.createElement("section", { className: hideEverything },
            React.createElement("section", { className: 'panel-heading agda-header-container' },
                ...
                , { 
                    ...
                    // style={toStyle(style)}
                    // hidden={inputMethod.activated || _.isEmpty(headerText)}
                    hidden: inputMethod.activated, isPending: pending, mountAt: mountAt.current === 1 /* Bottom */ ? 'bottom' : 'pane', onMountChange: (at) => {
