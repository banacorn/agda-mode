...
var initialState = /* array */[/* record */Block.record(["contents"], [Block.variant("L", 1, [/* array */[]])])];

function postprocess(stack, string) {
  var word = "";
  var escaped = false;
  var in_str = false;
  ...
  if (match$1 !== 0) {
    if (match$1 !== 1) {
      return /* Continue */Block.variant("Continue", 1, [(function (param) {
                    return postprocess(stack, param);
                  })]);
    } else {
...
function incrParse(string) {
  ...
  if (match.tag) {
    ...
    return postprocess(initialState, match[0]);
  }
}
