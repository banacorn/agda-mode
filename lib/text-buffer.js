...
var hole = require("./text-buffer/hole");
var getHoles = hole.getHoles;
var Goal = require("./text-buffer/goal");
var TextBuffer = (function () {
    ...
    TextBuffer.prototype.protectCursor = function (callback) {
        ...
        this.getCurrentGoal(position)
            ...
            return result;
        }).catch(err.OutOfGoalError, function () {
            _this.core.editor.setCursorBufferPosition(position);
            return result;
        });
    };
    ...
    TextBuffer.prototype.getCurrentGoal = function (cursor) {
        ...
        return new Promise(function (resolve, reject) {
            var goals = _this.goals.filter(function (goal) {
    ...
    TextBuffer.prototype.checkGoalContent = function (messages) {
        var _this = this;
        return function (goal) {
            var content = goal.getContent();
            if (content) {
                Promise.resolve(goal);
            }
            else {
                _this.core.panel.setContent(messages.title, [], "plain-text", messages.placeholder);
                _this.core.panel
                    .query()
                    .then(function (expr) {
                    if (expr) {
    ...
    TextBuffer.prototype.onGoalsAction = function (indices) {
        var _this = this;
        this.protectCursor(function () {
            var textRaw = _this.core.editor.getText();
            _this.removeGoals();
            getHoles(textRaw, indices).forEach(function (token) {
                var range = _this.core.editor.fromCIRange(token.originalRange);
                _this.core.editor.setTextInBufferRange(range, token.content);
                var goal = new Goal(_this.core.editor, token.goalIndex, token.modifiedRange);
