...
function InputMethod(Props) {
  ...
  var markers = match[0];
  var match$1 = ReactUpdate.useReducer(initialState, reducer);
  var send = match$1[1];
  var state = match$1[0];
  var stateRef = React.useRef(state);
  var match$2 = Hook$AgdaMode.useState("");
  var setReality = match$2[1];
  var reality = match$2[0];
  React.useEffect((function () {
          var match = Buffer$AgdaMode.next(state[/* buffer */1], reality);
          if (typeof match === "number") {
            Curry._1(send, /* Deactivate */1);
          } else if (match.tag) {
            var buffer = match[0];
            Curry._1(send, /* UpdateBuffer */Caml_chrome_debugger.simpleVariant("UpdateBuffer", [buffer]));
            var surface = Buffer$AgdaMode.toSurface(buffer);
            rewriteTextBuffer(editor, markers, surface);
          } else {
            Curry._1(send, /* UpdateBuffer */Caml_chrome_debugger.simpleVariant("UpdateBuffer", [match[0]]));
          }
          return undefined;
        }), /* array */[reality]);
