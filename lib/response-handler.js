    ...
    switch (response.kind) {
        ...
        case 'HighlightingInfo_Indirect':
            return Promise.promisify(fs.readFile)(response.filepath)
                .then(data => {
                const annotations = emacs_1.parseSExpression(data.toString()).map(emacs_1.parseAnnotation);
    ...
    switch (response.displayInfoKind) {
        ...
        case 'AllGoals':
            if (response.content.length === 0) {
                ...
                core.view.setJudgements(title, emacs_1.parseJudgements(response.content));
            }
            break;
        case 'AllWarnings':
            core.view.setAgdaError(emacs_1.parseError(response.content.join('\n')), true);
            break;
        case 'AllErrors':
            core.view.setAgdaError(emacs_1.parseError(response.content.join('\n')), true);
            break;
        case 'Auto':
            let solutions = emacs_1.parseSolutions(response.content);
            core.view.setSolutions(solutions);
            break;
        case 'Error':
            const error = emacs_1.parseError(response.content.join('\n'));
        ...
        case 'CurrentGoal':
            ...
            break;
        case 'GoalType':
            core.view.setJudgements('Goal Type and Context', emacs_1.parseJudgements(response.content));
            break;
        case 'ModuleContents':
        ...
        case 'WhyInScope':
            if (core.commander.currentCommand.kind === "GotoDefinition") {
                const result = emacs_1.parseWhyInScope(response.content);
                if (result) {
            ...
            break;
        case 'Context':
            core.view.setJudgements('Context', emacs_1.parseJudgements(response.content));
            break;
        case 'HelperFunction':
        case 'SearchAbout':
        case 'Intro':
