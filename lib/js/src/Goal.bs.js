...
function removeBoundary(self) {
  ...
  var content = Rebase.$$String.trim(self.textEditor.getTextInBufferRange(range));
...
function writeLambda(contents, self) {
  ...
  if (isLambdaWhere) {
    self.textEditor.setTextInBufferRange(rewriteRange, Rebase.$$String.joinWith("\n" + " ".repeat(indent), Rebase.List.fromArray(contents)));
    return /* () */0;
  } else {
    self.textEditor.setTextInBufferRange(rewriteRange, " " + Rebase.$$String.joinWith(" ; ", Rebase.List.fromArray(contents)));
    return /* () */0;
  }
}
...
function setContent(text, self) {
  var range = self.range.translate(new Atom.Point(0, 2), new Atom.Point(0, -2));
  var paddingSpaces = " ".repeat(Rebase.$$String.length(String(self.index)));
  return self.textEditor.setTextInBufferRange(range, " " + (text + (" " + paddingSpaces)));
}

function selectContent(self) {
  var indexWidth = Rebase.$$String.length(String(self.index));
