...
var Commander = (function () {
        ...
        if (this.loaded || _.includes(exception, command.kind)) {
            this.dispatchCommand(command)
                ...
                if (command.expectedGoalsActionReplies > 0) {
                    ...
                    _this.pendingCommandQueue.push({
                        kind: command.kind,
    ...
    Commander.prototype.resolvePendingCommand = function () {
        var command = _.last(this.pendingCommandQueue);
        if (command) {
            // deduct count by 1
            if (command.count > 0) {
                command.count -= 1;
    ...
    Commander.prototype.clearPendingCommand = function () {
        this.pendingCommandQueue = [];
    };
    ...
    Commander.prototype.computeNormalFormIgnoreAbstract = function () {
        ...
        return this.core.textBuffer.getCurrentGoal()
            .then(function (goal) {
            if (goal.isEmpty()) {
                return _this.core.view.query("Compute normal form (ignoring abstract)", [], 0 /* PlainText */, 'expression to normalize:')
    ...
    Commander.prototype.inputSymbolInterceptKey = function (kind, key) {
        ...
        return Promise.resolve({ status: 'Issued', command: kind });
    };
    return Commander;
}());
