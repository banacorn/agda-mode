// Generated by BUCKLESCRIPT VERSION 5.0.4, PLEASE EDIT WITH CARE
'use strict';

var Block = require("bs-platform/lib/js/block.js");
var Curry = require("bs-platform/lib/js/curry.js");
var React = require("react");
var Caml_obj = require("bs-platform/lib/js/caml_obj.js");
var ReactDom = require("react-dom");
var Caml_option = require("bs-platform/lib/js/caml_option.js");
var Hook$AgdaMode = require("../Hook.bs.js");
var Util$AgdaMode = require("../../Util.bs.js");
var Event$AgdaMode = require("../../Util/Event.bs.js");
var Settings__Debug$AgdaMode = require("./Settings__Debug.bs.js");
var Settings__Protocol$AgdaMode = require("./Settings__Protocol.bs.js");
var Settings__Breadcrumb$AgdaMode = require("./Settings__Breadcrumb.bs.js");
var Settings__Connection$AgdaMode = require("./Settings__Connection.bs.js");
var Settings__Placeholder$AgdaMode = require("./Settings__Placeholder.bs.js");

function at(x, y, classNames) {
  return Curry._1(Util$AgdaMode.ClassName[/* serialize */2], Util$AgdaMode.ClassName[/* addWhen */1]("hidden", Caml_obj.caml_notequal(x, y), classNames));
}

function Settings(Props) {
  var inquireConnection = Props.inquireConnection;
  var onInquireConnection = Props.onInquireConnection;
  var connection = Props.connection;
  var connectionError = Props.connectionError;
  var navigate = Props.navigate;
  var debug = Props.debug;
  var element = Props.element;
  var match = Hook$AgdaMode.useState(/* Root */0);
  var setURI = match[1];
  var uri = match[0];
  React.useEffect((function () {
          return Event$AgdaMode.onOk(setURI)(navigate);
        }), /* array */[]);
  var inDevMode = atom.inDevMode();
  if (element !== undefined) {
    return ReactDom.createPortal(React.createElement("section", {
                    className: "agda-settings",
                    tabIndex: -1
                  }, React.createElement(Settings__Breadcrumb$AgdaMode.make, {
                        uri: uri,
                        onNavigate: setURI
                      }), React.createElement("div", {
                        className: "agda-settings-pages"
                      }, React.createElement("ul", {
                            className: at(/* Root */0, uri, /* :: */Block.simpleVariant("::", [
                                    "agda-settings-menu",
                                    /* [] */0
                                  ]))
                          }, React.createElement("li", {
                                onClick: (function (param) {
                                    return Curry._1(setURI, /* Connection */1);
                                  })
                              }, React.createElement("span", {
                                    className: "icon icon-plug"
                                  }, "Connection")), React.createElement("li", {
                                onClick: (function (param) {
                                    return Curry._1(setURI, /* Protocol */4);
                                  })
                              }, React.createElement("span", {
                                    className: "icon icon-comment-discussion"
                                  }, "Protocol")), React.createElement("li", {
                                onClick: (function (param) {
                                    return Curry._1(setURI, /* UnicodeInput */2);
                                  })
                              }, React.createElement("span", {
                                    className: "icon icon-keyboard"
                                  }, "Unicode Input")), inDevMode ? React.createElement("li", {
                                  onClick: (function (param) {
                                      return Curry._1(setURI, /* Debug */3);
                                    })
                                }, React.createElement("span", {
                                      className: "icon icon-terminal"
                                    }, "Debug")) : null), React.createElement(Settings__Connection$AgdaMode.make, {
                            inquireConnection: inquireConnection,
                            onInquireConnection: onInquireConnection,
                            connection: connection,
                            error: connectionError,
                            hidden: uri !== /* Connection */1
                          }), React.createElement(Settings__Protocol$AgdaMode.make, {
                            hidden: uri !== /* Protocol */4
                          }), React.createElement(Settings__Placeholder$AgdaMode.make, {
                            hidden: uri !== /* UnicodeInput */2
                          }), React.createElement(Settings__Debug$AgdaMode.make, {
                            debug: debug,
                            hidden: uri !== /* Debug */3
                          }))), Caml_option.valFromOption(element));
  } else {
    return null;
  }
}

var URI = 0;

var make = Settings;

exports.URI = URI;
exports.at = at;
exports.make = make;
/* react Not a pure module */
