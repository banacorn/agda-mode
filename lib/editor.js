...
class Editor {
    ...
    jumpToRange(range) {
        this.focus();
        const bufferRange = range.intervals[0] ? new Atom.Range(new atom_1.Point(range.intervals[0].start[0], range.intervals[0].start[1]), new atom_1.Point(range.intervals[0].end[0], range.intervals[0].end[1])) : null;
        if (range.source) {
            // global
