...
function markerOnDidChange(editor, setReality, $$event) {
  ...
  var _oldBuffer = editor.getTextInBufferRange(rangeOld);
  var newBuffer = editor.getTextInBufferRange(rangeNew);
  if (_oldBuffer !== newBuffer) {
    console.log("[ IM ][ monitor ] \"" + (_oldBuffer + ("\" " + (String(rangeOld.end.column) + (" => \"" + (newBuffer + ("\" " + String(rangeNew.end.column))))))));
  }
  return Curry._1(setReality, newBuffer);
}
...
function InputMethod(Props) {
  ...
  React.useEffect((function () {
          ...
          if (typeof match === "number") {
            if (match === 0) {
              console.log("[ IM ][ reality ][ Noop ] ");
              Curry._1(setChangeLog, /* Noop */0);
            } else {
              console.log("[ IM ][ reality ][ Complete ]");
            ...
            switch (match.tag | 0) {
              case 0 : 
                  var buffer = match[0];
                  console.log("[ IM ][ reality ][ Insert ] " + Buffer$AgdaMode.toString(buffer));
                  Curry._1(setChangeLog, /* Noop */0);
                  Curry._1(send, /* UpdateBuffer */Caml_chrome_debugger.variant("UpdateBuffer", 0, [buffer]));
                  break;
              case 1 : 
                  var buffer$1 = match[0];
                  console.log("[ IM ][ reality ][ Backspace ] " + Buffer$AgdaMode.toString(buffer$1));
                  Curry._1(setChangeLog, /* Noop */0);
                  Curry._1(send, /* UpdateBuffer */Caml_chrome_debugger.variant("UpdateBuffer", 0, [buffer$1]));
                  break;
              case 2 : 
                  var buffer$2 = match[0];
                  console.log("[ IM ][ reality ][ Rewrite ] " + Buffer$AgdaMode.toString(buffer$2));
                  Curry._1(setChangeLog, /* Rewrite */1);
                  Curry._1(send, /* UpdateBuffer */Caml_chrome_debugger.variant("UpdateBuffer", 0, [buffer$2]));
                  var surface = Buffer$AgdaMode.toSurface(buffer$2);
                  rewriteTextBuffer(editor, markers, surface);
                  break;
              case 3 : 
                  console.log("[ IM ][ reality ][ Stuck ]", match[0]);
  ...
  React.useEffect((function () {
          if (hasChanged(state, changeLog)) {
            ...
            Event$AgdaMode.emitOk(/* () */0, onChange);
          }
          console.log(state[/* activated */0], Buffer$AgdaMode.toString(state[/* buffer */1]), state[/* reality */2]);
          if (atom.inDevMode()) {
  ...
  var bufferClassName = Curry._1(Util$AgdaMode.ClassName[/* serialize */2], Util$AgdaMode.ClassName[/* addWhen */1]("hidden", Buffer$AgdaMode.isEmpty(state[/* buffer */1]), /* :: */Caml_chrome_debugger.simpleVariant("::", [
              "inline-block",
