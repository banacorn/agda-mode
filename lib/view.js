...
class View {
    constructor(core) {
        ...
        this.emitter.on(actions_1.EVENT.JUMP_TO_RANGE, (range) => {
            this.core.editor.jumpToRange(range);
        });
        this.emitter.on(actions_1.EVENT.MOUSE_OVER, (range) => {
            this.core.editor.mouseOver(range);
        });
        this.emitter.on(actions_1.EVENT.MOUSE_OUT, (range) => {
    ...
    // for Emacs
    setEmacsAgdaError(error, isWarning = false) {
        this.store.dispatch(Action.MODE.display());
        this.editors.focusMain();
