...
function updateLatestEntry(f, self) {
  var n = Rebase.$$Array.length(self[/* entries */4]);
  return Rebase.$$Option.forEach(f, Rebase.$$Array.get(self[/* entries */4], n - 1 | 0));
}
...
function serialize$1(self) {
  ...
  var entries = Rebase.$$String.joinWith("\n", Rebase.List.fromArray(Rebase.$$Array.mapi(serialize, self[/* entries */4])));
  return "## Parse Log\n" + (String(path) + ("\n" + (String(args) + ("\n" + (String(version) + ("\n" + (String(protocol) + ("\n" + (String(os) + ("\n" + (String(entries) + "\n  ")))))))))));
}
