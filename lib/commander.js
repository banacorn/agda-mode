                ...
                if (Array.isArray(responses)) {
                    responses.forEach(response => {
                        try {
                            ...
                            handler_1.handleAgdaResponse(this.core, parser_1.parseAgdaResponse(response));
                        }
                        catch (error) {
                            // this.core.view.store.dispatch(Action.CONNECTION.err(this.selected.guid));
                            console.log(error);
                            // show some message
                            this.core.view.set('Agda Parse Error', [`Message from agda:`].concat(response), 3 /* Error */);
                        }
                    });
                }
