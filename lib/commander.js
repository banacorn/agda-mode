...
var Commander = (function () {
    ...
    Commander.prototype.inferType = function (normalization) {
        ...
        return this.core.textBuffer.getCurrentGoal()
            .then(function (goal) {
            if (goal.isEmpty()) {
                _this.core.view.setContent("Infer type " + toDescription(normalization), [], "value", "expression to infer:");
                _this.core.view.queryMode = true;
                return _this.core.view.query()
                    .then(function (expr) {
                    _this.core.process.inferType(normalization, expr, goal);
                })
                    .then(resolveCommand(12));
            }
            else {
    ...
    Commander.prototype.moduleContents = function (normalization) {
        var _this = this;
        this.core.view.setContent("Module contents " + toDescription(normalization), [], "plain-text", "module name:");
        return this.core.view.query()
            .then(function (expr) {
            return _this.core.textBuffer.getCurrentGoal()
                .then(function (goal) {
    ...
    Commander.prototype.computeNormalForm = function () {
        var _this = this;
        this.core.view.setContent("Compute normal form", [], "value", "expression to normalize:");
        return this.core.view.query()
            .then(function (expr) {
            return _this.core.textBuffer.getCurrentGoal()
                .then(function (goal) {
            ...
            .then(resolveCommand(14));
    };
    Commander.prototype.computeNormalFormIgnoreAbstract = function () {
        var _this = this;
        this.core.view.setContent("Compute normal form (ignoring abstract)", [], "value", "expression to normalize:");
        return this.core.view.query()
            .then(function (expr) {
            return _this.core.textBuffer.getCurrentGoal()
                .then(function (goal) {
    ...
    Commander.prototype.give = function () {
        ...
        return this.core.textBuffer.getCurrentGoal()
            .then(function (goal) {
            if (goal.isEmpty()) {
                _this.core.view.setContent("Give", [], "plain-text", "expression to give:");
                _this.core.view.query()
                    .then(function (expr) {
    ...
    Commander.prototype.case = function () {
        ...
        return this.core.textBuffer.getCurrentGoal()
            .then(function (goal) {
            if (goal.isEmpty()) {
                _this.core.view.setContent("Case", [], "plain-text", "expression to case:");
                _this.core.view.query()
                    .then(function (expr) {
                    goal.setContent(expr);
                    return goal;
    ...
    Commander.prototype.inputSymbol = function () {
        if (atom.config.get("agda-mode.inputMethod")) {
            if (!this.loaded) {
                this.core.atomPanel.show();
                this.core.view.setContent("Not loaded", [], "warning");
            }
