...
function next(self, input) {
  var underlying = self[/* underlying */2];
  var surfaceBody = self[/* surfaceBody */1];
  var surfaceHead = self[/* surfaceHead */0];
  var surface = toSurface(self);
  if (input === surface) {
    if (Translator$AgdaMode.translate(underlying)[/* further */1] && input !== "\\") {
